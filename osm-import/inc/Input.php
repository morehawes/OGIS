...
class OSM_Import_Input {
	
	private static $multi_value_seperator = '__multi__';
	...
	static public function create_field($field, $set_value = null, $show_label = true) {
		...
		if(array_key_exists('tip', $field)) {
			...
			if(array_key_exists('tip_link', $field)) {
				$out .= ' Click here for more details." href="' . $field['tip_link'] . '" target="_blank" class="waymark-tooltip waymark-link"';					
			} else {
	...
	static public function create_input($field, $set_value = null) {
		...
		if(array_key_exists('default', $field)) {
			if(is_array($field['default'])) {
				$field['default'] = implode(self::$multi_value_seperator, self::process_output($field, $field['default']));				
			} else {
		...
		if($set_value !== null) {
			if(is_array($set_value)) {
				$set_value = implode(self::$multi_value_seperator, self::process_output($field, $set_value));				
			} else {
		...
		switch($field['type']) {
			...
			case 'select_multi' :
				...
				} elseif(is_string($set_value) && strpos($set_value, self::$multi_value_seperator)) {
					$set_value = explode(self::$multi_value_seperator, $field['default']);
				}
...
//					$out .= ' value="' . htmlspecialchars($set_value) . '"';
	...
	static function create_repeatable_parameter_groups($name = 'repeatable', $fields, $groups = [], $repeatable_data = []) {
		...
		$out .= '<button class="button waymark-repeatable-add" title="Add Query"><i class="ion ion-plus"></i></button>';
