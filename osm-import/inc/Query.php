...
class OSM_Import_Query extends OSM_Import_Class {
...
// 	}	
	...
	function do_execute() {
		...
		//Build request
		$this->OverpassRequest = new OSM_Import_OverpassRequest([
			'Query' => $this
		]);							

		//Execute request
		$response = $this->OverpassRequest->get_processed_response();
...
// 		//Add
// 		if(sizeof($data)) {
// 			$out .= OSM_Import_Input::create_parameter_groups($this->inputs, $this->input_groups, $data);				
// 		//Edit
// 		} else {
// 			$out .= OSM_Import_Input::create_parameter_groups($this->inputs, $this->input_groups);
// 		}
// 
// 		$out .= '</div>' . "\n";
// 
...
// 
// 		//Bounds
// 		$query_area = OSM_Import_Config::get_setting('query', 'defaults', 'query_area_bounds');
// 		$query_area = explode(',', $query_area);
// 		$data['init_bounds'] = '[[' . $query_area[1] . ',' . $query_area[0] . '],[' . $query_area[3] . ',' . $query_area[2] . ']]';
// 
// 		//Set basemap
// 		if($editor_basemap = OSM_Import_Config::get_setting('misc', 'editor_options', 'editor_basemap')) {
// 			$data['basemap'] = $editor_basemap;		
// 		}
// 
// 		$OSM_Import_JS = new OSM_Import_Instance($data);
// 		$OSM_Import_JS->add_js();
// 		echo $OSM_Import_JS->get_html();	
// 	}
// 	
// 	function get_inputs() {
// 		return $this->inputs;
