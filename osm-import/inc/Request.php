<?php
abstract class OSM_Import_Request {
	protected $config;
	...
	function get_processed_response() {
		...
		//Determine cache ID
// 		$cache_id = 'Request_' . OSM_Import_Cache::do_hash($request);

		//Cached response	
		//Don't use cache in debug mode
//		if((! OSM_Import_Helper::is_debug()) && $cached_response = OSM_Import_Cache::get_item($cache_id)) {			
// 		if($cached_response = OSM_Import_Cache::get_item($cache_id)) {			
...
// 				$cache_minutes = OSM_Import_Config::get_setting('query', 'performance', 'cache_minutes');
// 						
// 				//Insert into cache
// 				OSM_Import_Cache::set_item($cache_id, $response_raw, $cache_minutes);			
// 			}
// 		}
